...
int main(){
	...
	do{
        ...
        switch(choice){
        	...
        	case 6:
        		sumNumbers=0;
        		printf("Cac so nguyen to co trong mang 2 chieu la\n");
        		for(int i=0;i<n;i++){
        			for(int j=0;j<m;j++){
        				isPrime=0;              // ktra numbers[i][j] ; 0 la so nguyen to ; 1 khong phai la so nguyen to
        				if(numbers[i][j]>=2){
				...
				printf("\nTong cac so nguyen to trong mang 2 chieu la %d\n",sumNumbers);
        		break;
        	case 7:
        		if(n==m){
